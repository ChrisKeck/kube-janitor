...
def send_delete_notification(
    ...
    message = f'{resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name} will be deleted at {formatted_expire_datetime} ({reason})'
    logger.info(message)
...
def delete(resource, wait_after_delete: int, dry_run: bool):
    if dry_run:
        logger.info(
            f'**DRY-RUN**: would delete {resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name}'
        )
    else:
        logger.info(
            f'Deleting {resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name}..'
        )
        try:
            ...
            logger.error(
                f'Could not delete {resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name}: {e}'
            )
    ...
    if ttl:
        try:
            ...
            if ttl_seconds > 0:
                ...
                logger.debug(
                    f'{resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name} with {ttl} TTL is {age_formatted} old'
                )
                if age.total_seconds() > ttl_seconds:
                    message = f'{resource.kind} {resource.namespace+"/" if resource.namespace else ""}{resource.name} with {ttl} TTL is {age_formatted} old and will be deleted ({reason})'
                    logger.info(message)
                    create_event(
